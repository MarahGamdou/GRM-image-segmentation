...
class PushRelabel:
    def __init__(self, graph, source, sink):
        ...
        for n in self.graph.neighbors(self.source):
            ...
            self.graph.update(
                nodes=[(n, {"excess": self.graph[self.source][n]["capacity"]})]
            )
            ...
            self.graph.update(
                nodes=[
                    (
                        self.source,
                        {
                            "excess": lexcess[self.source]
                            - self.graph[self.source][n]["capacity"]
                        },
                    )
                ]
            )
    ...
    def push(self, node):
        ...
        for v in self.graph.neighbors(node):
            ...
            if (self.graph[node][v]["capacity"] > self.graph[node][v]["flow"]) and (
                lheights[node] == lheights[v] + 1
            ):  # teste avec >
                flow = min(
                    self.graph[node][v]["capacity"] - self.graph[node][v]["flow"],
                    lexcess[node],
                )
    ...
    def relabel(self, node):
        ...
        lheights = nx.get_node_attributes(self.graph, "height")
        assert [
            lheights[node] <= lheights[v]
            for v in self.graph.neighbors(node)
            if self.graph[node][v]["capacity"] > self.graph[node][v]["flow"]
        ]
        self.graph.update(
            nodes=[
                (
                    node,
                    {
                        ...
                        + min(
                            [
                                lheights[v]
                                for v in self.graph.neighbors(node)
                                if self.graph[node][v]["capacity"]
                                > self.graph[node][v]["flow"]
                            ]
                        )
                    },
                )
